...
class AdobeStockController extends Controller
{
    public function search(Request $request)
    {
        ...
        try {
            ...
            $search_request->setLocale('en_US')
                           ...
                           ->setResultColumns($this->result_columns());

            // Initialize the files search
            $search_files_response = $client->searchFilesInitialize($search_request);

            $files = $search_files_response->getNextResponse()->files ?? [];

            return response()->json(array_map([$this, 'convert'], $files));
    ...
    {
        return is_array($image) ? $image[$prop] : $image->$prop;
    }
    ...
    {
        return $this->getProp($image, 'description') ?? $this->getProp($image, 'title');
    }
